...
% fc_f = inline('2*pi^2*cos(pi*x(1))*sin(pi*x(2))', 'x');
% % Dirichlet data, function g_D
% fc_gD = inline('cos(pi*x(1))*sin(pi*x(2))', 'x');
...
fc_f = inline('0', 'x');
% Dirichlet data, function g_D
% Handle r=0 case explicitly to avoid NaN/Inf with atan2(0,0) or r^(1/3)
fc_gD = inline(['( (x(1)^2+x(2)^2)^(1/3) .* sin(2/3 * atan2(x(2),x(1))) ) * (x(1)^2+x(2)^2>eps) + ' ...
              '0 * (x(1)^2+x(2)^2<=eps)' ], 'x');
